...
async function generateAnalise(){
    ...
    html+=`<button class="btn-ghost" id="btnBaixarPdf" onclick="downloadAnalisePDF({btn:this})">📄 Baixar em PDF</button>`;
    html+=`</div></div>`;
...
async function downloadAnalisePDF(opts={}){
    ...
    try{
        ...
        if(base64){
            ...
            // 2. Show link in chat bubble (auto) or transform button (manual)
            if(pdfUrl){
                if(isAuto){
                    showPdfLink(pdfUrl, company);
                } else if(opts.btn){
                    // Transform the download button to show link + copy icon inline
                    const safeUrl = pdfUrl.replace(/'/g,'%27');
                    const clipIcon = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`;
                    const wrapper = document.createElement('div');
                    wrapper.style.cssText = 'display:flex;flex-direction:column;gap:6px;margin-top:2px';
                    wrapper.innerHTML = `
                        <button class="btn-ghost" style="opacity:.6;cursor:default;pointer-events:none">✓ PDF gerado</button>
                        <div style="display:flex;align-items:center;gap:6px;background:rgba(107,190,208,.08);border:1px solid rgba(107,190,208,.25);border-radius:8px;padding:6px 10px;max-width:320px">
                            <a href="${pdfUrl}" target="_blank" rel="noopener" style="flex:1;color:#6bbed0;font-size:10px;text-decoration:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${pdfUrl}">🔗 ${pdfUrl.replace(/https?:\/\//,'')}</a>
                            <button onclick="navigator.clipboard.writeText('${safeUrl}').then(()=>{this.style.color='#22c55e';this.title='Copiado!';setTimeout(()=>{this.style.color='';this.title='Copiar link'},2000)})" title="Copiar link" style="background:none;border:none;cursor:pointer;color:#6bbed0;padding:2px;display:flex;align-items:center;flex-shrink:0">${clipIcon}</button>
                        </div>`;
                    opts.btn.replaceWith(wrapper);
                }
            }
